...
class ComputerService:
    ...
    async def list_computers(
        self, page: int = 1, page_size: int = 20, status: Optional[str] = None, group_id: Optional[str] = None, search: Optional[str] = None, organization_id: Optional[str] = None
    ) -> PaginatedResponse[ComputerResponse]:
        query = select(Computer)
        if organization_id:
            query = query.where(Computer.organization_id == organization_id)
        if status:
    ...
    async def create_group(self, data: ComputerGroupCreate, organization_id: Optional[str] = None) -> ComputerGroupResponse:
        group = ComputerGroup(
            name=data.name,
    ...
    async def update_group(self, group_id: str, data: ComputerGroupUpdate) -> ComputerGroupResponse:
        result = await self.db.execute(
            select(ComputerGroup).where(ComputerGroup.id == group_id)
        )
        ...
        await self.db.commit()
        await self.db.refresh(group)
        resp = ComputerGroupResponse.model_validate(group)
        count_result = await self.db.execute(
            select(func.count()).where(Computer.group_id == group.id)
        )
        resp.computer_count = count_result.scalar()
        return resp
